...
public class BarraVidaUIRene : MonoBehaviour
{
    [Header("Referencia")]
    ...
    private void Update()
    {
        if (InfoRene.instancia == null) return;

        float vidaActual = InfoRene.instancia.vida;

        if (!Mathf.Approximately(vidaActual, ultimaVida))
        {
            ActualizarBarra(vidaActual, InfoRene.instancia.vidaMaxima);
